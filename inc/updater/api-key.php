...
class TTF_Updater_API_Key {
	...
	public function add_tool_page() {
		add_submenu_page(
			'tools.php',
			__( 'Updater API Key', 'ttf-updater' ),
			__( 'Updater API Key', 'ttf-updater' ),
			'manage_options',
			'updater_auth_page',
	...
	?>
		<div class="wrap">
			<h2><?php _e( 'Updater Authorization', 'ttf-updater' ); ?></h2>
			...
			<form action="options.php" method="post">
					...
					<p class="submit" class="updater-toggle">
						<?php $label = ( true === $is_redirect ) ? __( 'Authorize Site and Complete Update', 'ttf-updater' ) : __( 'Authorize Site', 'ttf-updater' ); ?>
	...
	public function render_input_section() {
		...
		if ( ! empty( $api_key ) ) {
			printf(
				...
				__( 'API Key:', 'ttf-updater' ),
				$api_key
			);
			printf(
				'<p>%s</p>',
				__( 'Problems? <a href="#" class="updater-re-auth">Try authorizing again</a>.', 'ttf-updater' )
			);
		} else {
			printf(
				'<p>%s</p>',
				__( 'Please authorize your theme for automatic updates with the same credentials you use to sign into The Theme Foundry website. If you have trouble logging in, please <a href="https://thethemefoundry.com/support/">contact us in the Help Center</a>. The authorization process will send your current domain and product information to authorize this site.', 'ttf-updater' )
			);
		}
	?>
	...
	public function authorize_theme_and_domain( $value ) {
		...
		if ( 200 === $response_code && isset( $response_body->key ) ) {
			$msg = array(
				'status'  => 'updated',
				'message' => __( 'Your site is authorized for automatic updates from The Theme Foundry.', 'ttf-updater' ),
			);
			...
			if ( $key === preg_replace( '/[^a-z0-9]/i', '', $key ) ) {
				...
				$msg = array(
					'status'  => 'error',
					'message' => __( 'There was an error authorizing your site for automatic updates from The Theme Foundry.', 'ttf-updater' ),
					'details' => $errors,
				);
