...
class Bailey_Logo {
	...
	/**
	 * The one instance of Bailey_Logo
	 *
	 * @since 1.0.0.
	 *
	 * @var   Bailey_Logo    The one instance.
	 */
	...
	/**
	 * Instantiate or return the one Bailey_Logo instance.
	 *
	 * @since  1.0.0.
	 *
	 * @return Bailey_Logo
	...
	public function __construct() {
		add_action( 'bailey_css', array( $this, 'print_logo_css' ) );
	}
	...
	function get_logo_dimensions( $url, $force = false ) {
		// Build the cache key
		$key = 'bailey-' . md5( 'logo-dimensions-' . $url . BAILEY_VERSION );
	...
	function get_logo_information( $force = false) {
		...
		foreach ( $logos as $logo ) {
			$this->logo_information[ $logo ]['image'] = get_theme_mod( $logo, bailey_get_default( $logo ) );
		...
		$this->logo_information = apply_filters( 'bailey_custom_logo_information', $this->logo_information );
	...
	function print_logo_css() {
		$size = apply_filters( 'bailey_custom_logo_max_width', '1042' );
...
if ( ! function_exists( 'bailey_get_logo' ) ) :
/**
 * Return the one Bailey_Logo object.
 *
 * @since  1.0.0.
 *
 * @return Bailey_Logo
 */
function bailey_get_logo() {
	return Bailey_Logo::instance();
}
endif;

add_action( 'init', 'bailey_get_logo', 11 );

if ( ! function_exists( 'bailey_refresh_logo_cache' ) ) :
/**
 ...
 * @return void
 */
function bailey_refresh_logo_cache( $wp_customize ) {
	bailey_get_logo()->get_logo_information( true );
}
endif;

add_action( 'customize_save_after', 'bailey_refresh_logo_cache' );